...
export function ProjectGrid({ revealDelay = 0 }: ProjectGridProps) {
  ...
  return (
    <section className="soft-projects" aria-labelledby="projects-heading" data-intel="projects">
      <GlitchReveal variant="hero" delay={revealDelay}>
        <h2
          id="projects-heading"
          className="soft-projects__title soft-brand scene-headline glitch-idle"
          data-text="Projects"
        >
          Projects
        </h2>
      ...
      <ul className="soft-projects__grid">
        {projects.map((project, idx) => {
          const intel = projectIntelId(project.path);
          const card = (
            ...
            <li key={project.path}>
              <GlitchReveal variant="card" delay={revealDelay + 120 + idx * 95}>
                {project.externalHref ? (
                  ...
                  <Link to={project.path} className="soft-project-card" data-intel={intel}>
                    <span className="soft-project-card__glow" aria-hidden />
                    {card}
                  </Link>
                )}
